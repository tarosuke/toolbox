...
namespace tb {
	...
	Directory::Node::Node(const dirent& ent)
		: name(ent.d_name), type(ent.d_type), atime(0), mtime(0), ctime(0) {
	...
	Directory::Directory(const char* path, const Weight* w) { Load(path, w); }
				...
				if (auto* n = new Node(*ent)) {
					...
					for (List<Node>::I i(entries); ++i;) {
						...
						const float score =
							...
							weight.ctime *
								Sign((*n).GetCTime() - (*i).GetCTime()) +
							weight.mtime *
								Sign((*n).GetMTime() - (*i).GetMTime()) +
							weight.atime *
								Sign((*n).GetATime() - (*i).GetATime()) +
